...
class ParsingContentPage(BasePage):
    ...
    def _build_content_filter_section(self, section: CollapsibleSection):
        ...
        # 체크박스 변경 시 자동 저장 연결
        for cb in [self.include_dice, self.include_effects,
                   self.include_system, self.include_ooc,
                   self.merge_dialogue, self.images_enable, self.show_caption]:
    ...
    def _build_parsing_rules_section(self, section: CollapsibleSection):
        ...
        # --- 플랫폼 템플릿 카드 ---
        template_card = ContentCard("빠른 시작", "플랫폼과 룰 시스템을 선택하면 파싱 규칙이 자동 설정됩니다")
        ...
        self.separator_type = name_card.add_dropdown(
            ...
            options=["콜론 (:)", "탭 (Tab)", "대괄호 ([이름])", "꺾쇠 (<이름>)", "HTML 태그 자동", "직접 입력"],
        ...
        name_card.add_field("커스텀 구분자", self.custom_separator_input, "custom_separator_text",
        ...
        # --- 나레이션 인식 카드 ---
        narration_rule_card = ContentCard("나레이션 규칙", "나레이션(지문)으로 인식할 조건을 설정합니다")
    ...
    def load_settings(self):
        ...
        self.safe_set_combo(
            self.separator_type,
            str(_g('custom_separator_type', '콜론 (:)'))
        )
        if hasattr(self, 'custom_separator_input'):
            self.custom_separator_input.setText(
                str(_g('custom_separator_text', ''))
            )
            self._on_separator_type_changed(
                str(_g('custom_separator_type', '콜론 (:)'))
            )
        self.safe_set_combo(
            self.name_position,
            str(_g('custom_name_position', '구분자 앞 (이름: 대사)'))
        )
