...
class AdvancedSettingsPage(BasePage):
    ...
    def _build_output_section(self, section: CollapsibleSection):
        ...
        self.epub_page_size = epub_card.add_dropdown(
            ...
            options=[
                "EPUB (6x9)", "EPUB (5x8)",
                "A5 (148x210mm)", "신국판 (152x225mm)", "문고판 (105x148mm)"
            ],
        ...
        docx_card = ContentCard(
            "DOCX 설정", "Word 문서 페이지 크기 설정 (인쇄/편집용)",
    ...
    def _build_settings_manage_section(self, section: CollapsibleSection):
        ...
        manage_card = ContentCard("설정 관리", "모든 페이지의 설정을 저장하거나 JSON 파일로 관리합니다")
    ...
    def load_settings(self):
        ...
        self.safe_set_combo(
            self.epub_page_size,
            str(_g('epub_page_format', 'EPUB (6x9)'))
        )
        self.safe_set_combo(
            self.page_format,
            str(_g('page_format', 'A5 (148x210mm)'))
        )
        ...
        self.output_dir.setText(
            str(_g('output_dir', str(Path.cwd() / 'export')))
        )
        ...
        self.safe_set_combo(
            ...
            str(_g('image_jpeg_quality', '85 (권장)'))
        )
