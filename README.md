...
**저장소가 비공개(private)라면**: 자동 업데이트가 동작하지 않습니다 (GitHub API가 인증 없이 404 반환).
이 경우 앱은 다음과 같이 동작합니다:
- 시작 시 자동 체크는 1회 시도 후 자동 비활성화 (settings.json 에 기록됨)
- [업데이트 확인] 클릭 시 "저장소가 비공개입니다" InfoBar + **[Releases 페이지 열기]** 액션 버튼
- [정보] 메뉴 → **[새 버전 받기 (Releases)]** 버튼으로 언제든 브라우저에서 수동 다운로드

GitHub에 로그인된 브라우저면 private 저장소도 정상 접근 가능합니다.

</details>

<details>
<summary><b>비공개로 운영 중인데 자동 업데이트 알림이 거슬려요</b></summary>

`%APPDATA%\TRPG_Converter_Pro\gui_settings.json` 열고 추가하세요:
```json
"updates_check_on_startup": false
```

또는 앱이 첫 시작 시 1회 404를 감지하면 자동으로 위 플래그를 끕니다. 그 다음 부팅부터는 silent.

[업데이트 확인] 메뉴를 직접 누르면 그때만 1.5초 짧게 확인 (결과는 5분 캐시 → 연타해도 즉시 응답).
